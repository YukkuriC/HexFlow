...
class OpCubeFor(override val isPure: Boolean) : AbstractThoth() {
    override fun getData(): Pair<SpellList, Int> {
        ...
        val pos1 = stack.getBlockPos(lastPos - 1, size)
        val pos2 = stack.getBlockPos(lastPos, size)
        ...
        return Pair(SpellList.LList(pts.map(::Vec3Iota)), usedArgc)
    }
}